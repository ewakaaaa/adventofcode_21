    ...
    for i in graph[point]:
        if point.islower():
            if point == small_letter:
                if small_counter == 0:
                    paths.update(
                        solution(graph, i, visited.copy(), path, small_letter, 1)
                    )
                if small_counter == 1:
                    visited.add(point)
                    paths.update(
                        solution(graph, i, visited.copy(), path, small_letter, 2)
                    )
            else:
                visited.add(point)
                paths.update(
                    solution(
                        graph, i, visited.copy(), path, small_letter, small_counter
                    )
                )
        else:
            paths.update(
                solution(graph, i, visited.copy(), path, small_letter, small_counter)
            )
    return paths
